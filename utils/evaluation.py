...
def NCP(att_trees, result, sa=True):
    """
...
def NCP_record(att_trees, record, qi_len, qi_range):
    ...
    for i in range(qi_len):
        ...
        width = 0.0
        if isinstance(att_trees[i], NumRange) is False:
...
def get_result_cover(att_trees, result):
    ...
    for record in result:
        ...
        cover_set.append(record[-1])
        gen_result.append(cover_set)
    return gen_result


def get_result_cover_1m(att_trees, result):
    init_cover_dict(att_trees)
    qi_len = len(result[0]) - 1
    gen_result = []
    for record in result:
...
def count_query(data, att_select, value_select):
    ...
    for record in data:
        # sa check
        if record[-1] not in set(value_select[-1]):
            continue
        # qi check
...
def average_relative_error(att_trees, data, result, qd=DEFAULT_QD, s=DEFAULT_S):
    ...
    qd denotes the query dimensionality, b denotes selection of query
    """
    if _DEBUG:
        print "qd=%d s=%d" % (qd, s)
